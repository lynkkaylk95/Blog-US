...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/reset-password", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token, password }), signal: controller.signal }); window.clearTimeout(timeout);
      const result = await response.json() as { message?: string };
      if (response.ok) { setComplete(true); setMessage(t("resetSuccess")); } else setMessage(result.message || t("resetFailed"));
    } catch { setMessage(t("resetFailed")); }
