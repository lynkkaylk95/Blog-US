...
export function PostEditor({ postId, seriesMode = false, initialSeriesTitle = "", initialPartNumber = 1, initialCategories = ["Series"], initialAuthor = "Porchlight Editors", initialImageUrl = "" }: { postId?: number; seriesMode?: boolean; initialSeriesTitle?: string; initialPartNumber?: number; initialCategories?: string[]; initialAuthor?: string; initialImageUrl?: string }) {
  ...
  function partNumberChanged(partNumber: number) { setPost((current) => ({ ...current, partNumber, slug: seriesSlug({ ...current, partNumber }, current.title) })); }
  ...
  return <main className="admin-main">
    ...
    <form className="admin-editor-form" onSubmit={submit}>
      <section className="admin-panel admin-fields">
        ...
        {isSeries && <div className="field"><label>{postId ? t("currentPart") : t("partNumber")}</label>{postId ? <input value={`Part ${post.partNumber || 1}`} readOnly /> : <input type="number" min="1" step="1" value={post.partNumber || 1} onChange={(event) => partNumberChanged(Math.max(1, Number.parseInt(event.target.value, 10) || 1))} />}</div>}
