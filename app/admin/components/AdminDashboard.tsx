...
export function AdminDashboard() {
  ...
  return <main className="admin-main">
    ...
    <section className="admin-panel"><div className="admin-filters"><input type="search" placeholder={t("searchPlaceholder")} value={query} onChange={(event) => setQuery(event.target.value)} /><select value={filter} onChange={(event) => setFilter(event.target.value)}><option value="all">{t("allStatuses")}</option><option value="published">{t("published")}</option><option value="draft">{t("draft")}</option></select><span>{visible.length} {t("posts").toLowerCase()}</span></div>{message && <div className="admin-notice" role="status">{message}</div>}<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t("story")}</th><th>{t("author")}</th><th>{t("views")}</th><th>{t("category")}</th><th>{t("updated")}</th><th>{t("status")}</th><th>{t("actions")}</th></tr></thead><tbody>{visible.map((post) => <tr key={post.id}><td><div className="admin-post-title"><img src={post.imageUrl} alt="" /><div><b>{post.title}</b><small>/{post.slug}</small></div></div></td><td>{post.author}</td><td>{post.views.toLocaleString(locale === "vi" ? "vi-VN" : "en-US")}</td><td>{post.category}</td><td>{new Date(post.updatedAt).toLocaleDateString(locale === "vi" ? "vi-VN" : "en-US")}</td><td><span className={`status status--${post.status}`}>{t(post.status)}</span></td><td><div className="admin-actions">{post.status === "published" && <Link title={t("view")} href={`/story/${post.slug}`} target="_blank"><AdminIcon name="eye" /></Link>}<button title={t("copyLink")} onClick={() => copy(post)}><AdminIcon name="copy" /></button><Link title={t("edit")} href={`/admin/posts/${post.id}`}><AdminIcon name="edit" /></Link><button className="danger" title={t("delete")} onClick={() => remove(post)}><AdminIcon name="trash" /></button></div></td></tr>)}</tbody></table>{!message && !visible.length && <div className="admin-empty">{t("noPosts")}</div>}</div></section>
  </main>;
}
